...
class BypassTrackingCallback(TrainerCallback):
    ...
    def __init__(self, reward_func, output_dir: str, top_k: int = 2, judge_threshold: float = 0.5):
    ...
    def on_step_end(self, args, state, control, model=None, **kwargs):
        ...
        if hasattr(self.reward_func, 'last_step_metrics'):
            ...
            bypass_score = self.compute_bypass_score(quality_bypass_rate, avg_quality_judge_score)
    ...
    def _maybe_save_checkpoint(self, step: int, score: float, model):
        ...
        if len(self.top_models) < self.top_k:
            ...
            print(f"💾 Saved checkpoint (top {len(self.top_models)}) - step {step}, score={score:.4f}")
        else:
            ...
            if score > worst_score:
                ...
                if Path(worst_path).exists():
                    ...
                    print(f"🗑️  Removed checkpoint step {worst_step} (score={worst_score:.4f})")
                
                # Save new checkpoint
                self._save_model(model, checkpoint_path)
                heapq.heappush(self.top_models, (score, step, str(checkpoint_path)))
                print(f"💾 Saved checkpoint (top {self.top_k}) - step {step}, score={score:.4f}")
    ...
    def print_summary(self):
        ...
        print(f"    Quality bypasses: {best_step['quality_bypass_count']}/{best_step['total_samples']} ({best_step['quality_bypass_rate']:.1%})")
...
class ModeCollapseEarlyStoppingCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if step >= self.min_steps:
            ...
            if self.high_similarity_streak >= self.patience:
                ...
                print(f"   Similarity has been ≥{self.similarity_threshold:.2f} for {self.high_similarity_streak} consecutive steps")
                print(f"   Recent similarities: {[f'{s:.3f}' for s in self.similarity_history[-5:]]}")
            
            # Zero bypass streak: model gave up
            elif self.zero_bypass_streak >= self.bypass_patience:
                self.stopped_early = True
                self.stop_reason = f"zero_bypasses (0 bypasses for {self.bypass_patience} steps)"
                control.should_training_stop = True
                print(f"\n🛑 EARLY STOPPING: No bypasses detected!")
                print(f"   Model has produced 0 bypasses for {self.zero_bypass_streak} consecutive steps")
    ...
    def print_summary(self):
        ...
        if self.similarity_history:
            ...
            print(f"  Final 5 similarities: {[f'{s:.3f}' for s in self.similarity_history[-5:]]}")
...
class Profiler:
    ...
    def record_generation_time(self, num_tokens: int):
        ...
        if self.last_reward_time is not None:
            ...
            if self.timings.get("reward_total"):
                last_reward = self.timings["reward_total"][-1] if self.timings["reward_total"] else 0
    ...
    def log_step(self, total_tokens: int = 0, total_samples: int = 0):
        ...
        if self.step_count % self.print_every == 0:
            print(f"\n{'─'*60}")
            print(f"⏱️  PROFILING (Step {self.step_count}, avg of last {self.print_every} steps)")
            print(f"{'─'*60}")
            ...
            for name in ["grpo_gen+train", "detector", "judge", "reward_total"]:
                ...
                if times:
                    recent = times[-self.print_every:] if len(times) >= self.print_every else times
                    ...
                    if name in self.token_counts and self.token_counts[name]:
                        recent_tokens = self.token_counts[name][-self.print_every:] if len(self.token_counts[name]) >= self.print_every else self.token_counts[name]
                        avg_tokens = sum(recent_tokens) / len(recent_tokens)
                        tps = avg_tokens / avg_time if avg_time > 0 else 0
                        print(f"  {name:20s}: {avg_time:6.2f}s | {tps:,.0f} tok/s | {avg_tokens:,.0f} tokens")
                    else:
                        print(f"  {name:20s}: {avg_time:6.2f}s")
            
            # Calculate total step time
            if "grpo_gen+train" in component_times and "reward_total" in component_times:
                step_time = component_times["grpo_gen+train"] + component_times["reward_total"]
                print(f"  {'─'*40}")
                ...
                if total_samples > 0:
                    ...
                    print(f"  {'Est. time/1000 samp':20s}: {1000/total_samples*step_time/60:.1f} min")
                ...
                for name, times in self.timings.items():
                    if times:
                        ...
                        if name in self.token_counts and self.token_counts[name]:
                            ...
                            avg_tokens = sum(recent_tokens) / len(recent_tokens)
                            avg_time = sum(recent) / len(recent)
                            wandb_data[f"profiling/{name}_tps"] = avg_tokens / avg_time if avg_time > 0 else 0
                if "grpo_gen+train" in component_times and "reward_total" in component_times:
                    step_time = component_times["grpo_gen+train"] + component_times["reward_total"]
                    wandb_data["profiling/step_time"] = step_time
                    if total_samples > 0:
                        wandb_data["profiling/samples_per_sec"] = total_samples / step_time
...
class DetectorOnlyReward:
    ...
    def __call__(self, prompts, completions, **kwargs) -> list[float]:
        ...
        if self.step_count % self.print_every == 0 and responses:
            ...
            print(f"Bypass rate (this step): {bypass_rate:.1%} ({bypass_count_this_step}/{len(responses)})")
...
"""
    ...
    def _get_detector_scores(self, responses: list[str]) -> tuple[list[float], list[float]]:
    ...
    def _compute_diversity_penalties(self, responses: list[str]) -> list[float]:
        ...
        with torch.no_grad():
            ...
            k_hot = torch.zeros((n, vocab_size), dtype=torch.float32, device=self.device)
            ...
            penalties = torch.clamp((avg_similarities - 0.1) / 0.4, min=0.0, max=1.0) * self.diversity_weight
    ...
    def _get_judge_scores(self, responses: list[str]) -> list[float]:
        ...
        for i in range(0, len(all_texts), BATCH_SIZE):
            ...
            for j, output in enumerate(outputs):
                input_len = inputs.input_ids[j].ne(self.judge_tokenizer.pad_token_id).sum().item()
            ...
            for j, output in enumerate(outputs):
                ...
                response = self.judge_tokenizer.decode(generated, skip_special_tokens=True)
    ...
    def __call__(self, prompts, completions, **kwargs) -> list[float]:
        ...
        total_completion_tokens = sum(
            len(self.detector_tokenizer.encode(c[0]["content"], add_special_tokens=False))
        ...
        rolling_bypass_rate = sum(self.bypass_history) / len(self.bypass_history) if self.bypass_history else 0.0
        ...
        for ham_prob, spam_prob, judge_score in zip(ham_probs, spam_probs, judge_scores):
            ...
            quality_bonus = ham_prob * judge_score * self.judge_weight * self.current_judge_scale if self.use_judge else 0.0
        ...
        for i, (response, prompt, ham_prob, spam_prob, total_reward, judge_score) in enumerate(
            zip(responses, prompt_texts, ham_probs, spam_probs, total_rewards, judge_scores)
        ):
        ...
        current_bypass_rate = bypass_count_this_step / len(responses) if responses else 0.0
        ...
        avg_diversity_penalty = sum(diversity_penalties) / len(diversity_penalties) if diversity_penalties else 0.0
        ...
        self.last_step_metrics = {
            ...
            'avg_quality_judge_score': sum(quality_judge_scores) / len(quality_judge_scores) if quality_judge_scores else 0.5,
        ...
        if self.step_count % self.print_every == 0 and responses:
            ...
            msg = responses[0]
            print(f"SMS: {msg[:200]}..." if len(msg) > 200 else f"SMS: {msg}")
            print(f"Detector: HAM={ham_probs[0]:.2%}, SPAM={spam_probs[0]:.2%} → {detector_components[0]:.3f}")
            if self.use_judge:
                print(f"Judge: {judge_scores[0]:.2f} → {quality_components[0]:.3f}")
            print(f"Total reward: {total_rewards[0]:.3f}")
            print(f"Bypass: {bypass_count_this_step}/{len(responses)} ({bypass_rate:.1%})")
            print(f"Quality bypass (judge≥0.5): {quality_bypass_count}/{len(responses)} ({quality_rate:.1%})")
            print(f"📊 Scheduler: rolling_bypass={rolling_bypass_rate:.1%}, boost={self.current_bypass_boost:.2f}, judge_scale={self.current_judge_scale:.2f}")
            print(f"Diversity: similarity={avg_similarity:.3f}, penalty={avg_diversity_penalty:.3f}")
        ...
        profiler.log_step(total_tokens=total_completion_tokens, total_samples=len(responses))
...
def train_rl_phase(
    ...
    """
    print("=" * 60)
    print("RL Phase: " + ("Combined Detector + LLM Judge" if use_judge else "Detector-Only Reward"))
    print("=" * 60)
