...
URL_PATTERN = re.compile(
    r"https?://\S+|www\.\S+|\S+\.(com|net|org|io|co|info|biz)\b", re.IGNORECASE
)
...
US_PHONE_PATTERN = re.compile(
    r"\b\d{3}[-.\s]\d{3}[-.\s]\d{4}\b|\b\(\d{3}\)\s*\d{3}[-.\s]?\d{4}\b"
)

# Short code / reply keyword patterns (common in SMS spam)
SHORT_CODE_PATTERN = re.compile(
    r"\b(text|reply|send|msg)\s+\w+\s+(to\s+)?\d{4,6}\b", re.IGNORECASE
)
REPLY_KEYWORD_PATTERN = re.compile(
    r"\b(reply|text|send)\s+(stop|yes|no|help|info|start|join|quit)\b", re.IGNORECASE
)
...
CALL_ACTION_PATTERN = re.compile(
    r"\b(call|dial|ring|contact)\s*(us|me|now|back|today|this)?\b", re.IGNORECASE
)
...
class RewardFunctions:
    ...
    def detector_reward(self, prompts, completions, **kwargs) -> list[float]:
        ...
        if self.step_count % self.print_every == 0:
            valid_bypass = sum(
                1
                for r, hp in zip(responses, ham_probs.tolist())
            ...
            avg_detector_ms = (
                self.detector_time_total / max(self.detector_calls, 1)
            ) * 1000
            ...
            print(
                f"Valid bypass rate: {bypass_rate:.1%} ({valid_bypass}/{len(responses)})"
            )
            print(f"Total valid bypasses logged: {self.bypass_count}")
            print(
                f"⏱️  Avg detector: {avg_detector_ms:.1f}ms | Avg reward total: {avg_reward_ms:.1f}ms"
            )
            print(f"{'=' * 50}")
    ...
    def print_profiling_summary(self):
        ...
        print(f"{'=' * 60}")
        print("Detector inference:")
        print(f"  - Total time: {self.detector_time_total:.2f}s")
        print(f"  - Calls: {self.detector_calls}")
        print(
            f"  - Avg per call: {(self.detector_time_total / max(self.detector_calls, 1)) * 1000:.1f}ms"
        )
        print("Reward computation (total):")
        print(f"  - Total time: {self.reward_time_total:.2f}s")
        print(f"  - Calls: {self.reward_calls}")
        print(
            f"  - Avg per call: {(self.reward_time_total / max(self.reward_calls, 1)) * 1000:.1f}ms"
        )

        # Calculate detector percentage of total reward time
        detector_pct = (
            self.detector_time_total / max(self.reward_time_total, 0.001)
        ) * 100
        print(f"Detector % of reward time: {detector_pct:.1f}%")
        print("Note: Generation time is tracked by GRPOTrainer (not shown here)")
        print(f"{'=' * 60}")
