...
def train(msg: Message, context: Context) -> Message:
    ...
    log.info(
        f"[P{partition_id}] Data loaded in {time.time() - t0:.2f}s - {len(trainloader.dataset)} samples, {len(trainloader)} batches (bs={batch_size})"
    )
    ...
    if use_class_weights:
        ...
        log.info(
            f"[P{partition_id}] Class weights: HAM={class_weights[0]:.2f}, SPAM={class_weights[1]:.2f}"
        )

    # Call the training function
    log.info(
        f"[P{partition_id}] Starting training: {context.run_config['local-epochs']} epochs, lr={msg.content['config']['lr']}"
    )
    t0 = time.time()
    train_loss = train_fn(
        model,
    ...
    log.info(
        f"[P{partition_id}] Training completed in {training_time:.2f}s - loss: {train_loss:.4f}"
    )
    ...
    log.info(
        f"[P{partition_id}] === TRAIN END === Total: {time.time() - start_time:.2f}s"
    )
...
def evaluate(msg: Message, context: Context) -> Message:
    ...
    log.info(
        f"[P{partition_id}] Eval data loaded in {time.time() - t0:.2f}s - {len(valloader.dataset)} samples"
    )
    ...
    log.info(
        f"[P{partition_id}] === EVAL END === Total: {time.time() - start_time:.2f}s"
    )
    return Message(content=content, reply_to=msg)
