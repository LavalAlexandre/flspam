...
# Business types
BUSINESS_TYPES = [
    "pharmacy", "restaurant", "pizza_delivery", "grocery_store", "bank",
    "gym", "salon", "auto_shop", "dry_cleaner", "pet_store",
    "dentist_office", "medical_clinic", "school", "daycare", "veterinary_clinic",
]
...
def load_personas(n: int = 1500, min_age: int = MIN_AGE) -> list[dict[str, Any]]:
    ...
    """
    print(f"Loading {n} adult personas (age >= {min_age}) from nvidia/Nemotron-Personas-USA...")
...
def _has_shared_hobbies(persona1: dict[str, Any], persona2: dict[str, Any], min_shared: int = 1) -> bool:
    """
    ...
    for relationship_type, count in distribution.items():
        ...
        for _ in range(count):
            ...
            if relationship_type == "colleagues":
                ...
                filter_fn = lambda p: p.get("occupation", "") == main_occupation and main_occupation != ""
            elif relationship_type == "partner":
                # Partner should be adult (already filtered) and not same person
                filter_fn = lambda p: p.get("marital_status", "").lower() in MARRIED_STATUSES
                ...
                filter_fn = lambda p, mh=min_hobbies: _has_shared_hobbies(main_persona, p, min_shared=mh)
                ...
                shuffled_services = random.sample(PROFESSIONAL_TYPES, len(PROFESSIONAL_TYPES))
                for service_type in shuffled_services:
                    filter_fn = lambda p, st=service_type: _occupation_matches_service(
                        p.get("occupation", ""), st
                    )
                    candidate = _select_random_from_pool(persona_pool, used_personas, filter_fn)
                ...
                if matched_service_type is not None:
                    ...
                    contact.business_name = f"{location} {contact.service_type.title()} Services"
            ...
            contact_persona = _select_random_from_pool(persona_pool, used_personas, filter_fn)
            
            if contact_persona is None:
                # If no matching colleague found, try without filter for colleagues
                if relationship_type == "colleagues":
                    contact_persona = _select_random_from_pool(persona_pool, used_personas)
            ...
            if relationship_type == "professionals":
                ...
                contact.business_name = f"{location} {contact.business_type.replace('_', ' ').title()}"
...
def print_network_summary(network: PersonaNetwork) -> None:
    ...
    print(f"  Location: {network.main_persona.get('city')}, {network.main_persona.get('state')}")
    ...
    for rel_type, contacts in network.contacts.items():
        ...
        for c in contacts[:3]:  # Show first 3 of each type
            if c.service_type:
                ...
                print(f"  - {c.business_name} ({c.service_type}) - occupation: {occupation}")
            elif c.business_type:
                print(f"  - {c.business_name} ({c.business_type})")
            elif rel_type == "partner":
                print(f"  - Age: {c.persona.get('age')}, Occupation: {c.persona.get('occupation')}")
            elif rel_type == "colleagues":
                print(f"  - Age: {c.persona.get('age')}, Occupation: {c.persona.get('occupation')}")
            else:
                print(f"  - Age: {c.persona.get('age')}, Location: {c.persona.get('city')}, {c.persona.get('state')}")
