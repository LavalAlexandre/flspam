...
subprocess.check_call(
    [sys.executable, "-m", "pip", "install", "-q", "pydantic", "distilabel[vllm]"]
)
...
from distilabel.steps import Step, step  # noqa: E402
...
def compute_quality_score(conv: dict) -> tuple[float, list[str]]:
    ...
    if repeated:
        score -= 0.3
        issues.append("repetitive_phrases")
    ...
    for text in texts:
        non_ascii = [
            c for c in text if ord(c) > 127 and not (0x1F300 <= ord(c) <= 0x1F9FF)
        ]
        if len(non_ascii) > 3:
            score -= 0.1
...
class SMSQualityJudge(Step):
    ...
    def _build_judge_prompt(self, conv: dict) -> dict:
        ...
        conv_text = "\n".join(
            f"[{m.get('sender_uuid', '')[:8]}]: {m['text']}" for m in messages
        )
...
def run_quality_filter(
    ...
    print(
        f"  Filtered: {len(filtered)} ({100 * len(filtered) / len(conversations):.1f}%)"
    )
