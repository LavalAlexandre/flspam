...
def clean_message(text: str) -> str:
    ...
    text = re.sub(
        r"^(?:Dear|Hi|Hello|Hey)\s+[\w\s]+,\s*", "", text, flags=re.IGNORECASE
    )
...
def check_message_quality(
    text: str, history: list[dict], rel_type: str
) -> tuple[bool, str]:
    """
...
def should_end_conversation(history: list[dict]) -> bool:
    ...
    confirm_count = sum(
        1 for msg in last_msgs if any(p in msg for p in confirm_patterns)
    )
    if confirm_count >= 2:
        return True
    ...
    if len(history) >= 3:
        ...
        if any(
            count >= 3 for phrase, count in phrase_counts.items() if len(phrase) > 5
        ):
            return True
    ...
    if len(history) >= 4:
        ...
        for i in range(len(recent_words) - 1):
            if recent_words[i] and recent_words[i + 1]:
                overlap = len(recent_words[i] & recent_words[i + 1]) / max(
                    len(recent_words[i] | recent_words[i + 1]), 1
                )
                if overlap > 0.6 and i >= 1:
                    prev_overlap = len(recent_words[i - 1] & recent_words[i]) / max(
                        len(recent_words[i - 1] | recent_words[i]), 1
                    )
                    if prev_overlap > 0.5:
                        return True
...
def infer_sms_style(persona: dict) -> str:
    ...
    if age < 25:
        ...
        traits.extend(
            ["formal", "complete sentences", "no emoji", "may sign off with name"]
        )
