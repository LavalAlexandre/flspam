...
from src.model.modernbert import create_model, get_tokenizer
...
def distribute_spam(
    ...
    adversarial_spam = [
        msg for msg in spam_messages if msg.get("source") == "adversarial"
    ]
...
def compute_class_weights(trainloader: DataLoader) -> list[float]:
    ...
    weights = [
        total / (n_classes * label_counts[0])
        if label_counts[0] > 0
        else 1.0,  # HAM weight
        total / (n_classes * label_counts[1])
        if label_counts[1] > 0
        else 1.0,  # SPAM weight
    ]
...
def train(
    ...
    if class_weights is not None:
        ...
        log.info(
            f"Using class weights: HAM={class_weights[0]:.2f}, SPAM={class_weights[1]:.2f}"
        )
    else:
        ...
        for batch_idx, batch in enumerate(trainloader):
            ...
            with torch.amp.autocast(device.type, enabled=use_amp):
                outputs = model(
                    ...
                    labels=None
                    if criterion
                    else labels,  # Don't pass labels if using custom loss
                )
            ...
            if num_batches % log_interval == 0 or num_batches == total_batches:
                ...
                log.info(
                    f"  Batch {num_batches}/{total_batches} (epoch {epoch + 1}/{epochs}) - loss: {avg_loss:.4f}, lr: {lr:.2e}"
                )
